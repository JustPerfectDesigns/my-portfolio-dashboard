import { List } from "phosphor-react";
import Link from "next/link";
import { useRef } from "react";

import Image from "next/image";
import { useRouter } from "next/router";

import { BsGear } from "react-icons/bs";

import sidebarNav from "../data/sidebarNav";

export default function navbar() {
	const sideBarRef = useRef();

	function toogleSideBar() {
		sideBarRef.current.classList.toggle("-translate-x-full");
	}

	const router = useRouter();

	return (
		<div className="max-h-screen md:sticky md:top-0 z-50 text-white">
			{/* MOBILE SIDEBAR */}
			<div className="bg-purple-800 flex justify-between p-2 items-center sticky top-0 z-30">
				<a className="block text-white font-extrabold dark:text-dw ">CoolBar</a>
				<button className="rounded focus:bg-purple-800" onClick={toogleSideBar}>
					<List size={32} />
				</button>
			</div>
			{/* MAIN SIDEBAR */}
			<div
				ref={sideBarRef}
				className="bg-purple-800 w-48 space-y-10 px-5 py-7  absolute inset-y-0 left-0 transform -translate-x-full
         md:relative md:translate-x-0 z-50 transition duration-200 ease-in-out flex flex-col child:transition-all md:max-h-screen md:min-h-screen md:sticky md:top-0"
			>
				<div className="logo">
					<Image
						src="/../public/images/ideasmarketplacelogo.png"
						width={100}
						height={100}
						alt="Ideas Marketplace Logo"
						className="mb-12 w-[150px]"
					/>
				</div>

				{/* <nav className="flex flex-col grow space-y-3">
					<Link
						href="/home"
						passHref
						className="flex flex-row py-2.5 px-4 transition duration-200 rounded items-center space-x-3 hover:bg-blue-600 hover:text-white"
					>
						<HouseSimple size={20} />
						<p>Home</p>
					</Link>

					<Link href="/stats">
						<div className="flex flex-row py-2.5 px-4 transition duration-200 rounded items-center space-x-3 hover:bg-blue-600 hover:text-white">
							<ChartPieSlice size={20} />
							<p>Stats</p>
						</div>
					</Link>
					<Link
						href="/donate"
						passHref
						className="flex flex-row py-2.5 px-4 transition duration-200 rounded items-center space-x-3 hover:bg-blue-600 hover:text-white"
					>
						<Coins size={20} />
						<p>Donate</p>
					</Link>
					<Link
						href="/feedback"
						passHref
						className="flex flex-row py-2.5 px-4 transition duration-200 rounded items-center space-x-3 hover:bg-blue-600 hover:text-white"
					>
						<Lifebuoy size={20} />
						<p>Help</p>
					</Link>
				</nav> */}

				<div className="flex flex-col justify-between h-4/5">
					<div className="sidebar__top">
						<span className="menu-desc block text-[.65rem] uppercase mb-5 text-navlink-color">
							Menu Items
						</span>
						<ul className="menu__items grid gap-8">
							{sidebarNav.map((nav, index) => (
								<li
									key={`nav-${index}`}
									className={
										router.pathname === nav.link
											? "text-green-color font-semibold"
											: "text-navlink-color"
									}
								>
									<Link href={nav.link} passHref>
										<div className="navLink flex items-center gap-3">
											<div className="navLink-icon text-base text-green-color">
												{nav.icon}
											</div>
											<span className="navLink-name block text-sm">
												{nav.text}
											</span>
										</div>
									</Link>
								</li>
							))}
						</ul>
					</div>

					<div className="sidebar__middle w-full h-[1px] bg-black-border"></div>
					<div className="sidebar__bottom">
						<span className="menu-desc block text-[.65rem] uppercase mb-5 text-navlink-color">
							Others
						</span>
						<div className="menu__items">
							<div className="navLink flex items-center gap-3">
								<BsGear className="navLink-icon text-base text-green-color" />
								<span className="navLink-name block text-sm font-semibold text-navlink-color">
									Settings
								</span>
							</div>
						</div>
					</div>
				</div>

				{/* <button
					onClick={console.log("signout")}
					color="error"
					className="flex flex-row w-full items-center  rounded py-2.5 my-8 space-x-2 hover:bg-red-500 hover:text-white transition duration-200"
				>
					<SignOut className=" mx-2" size={20} />
					<a>Logout</a>
				</button> */}
			</div>
		</div>
	);
}
